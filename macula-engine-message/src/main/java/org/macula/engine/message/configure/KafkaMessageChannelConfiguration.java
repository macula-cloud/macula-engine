...
public class KafkaMessageChannelConfiguration {
	...
	@Bean
	KafkaMessageChannelExchange kafkaMessageChannelExchange(StreamBridge streamBridge) {
	...
	@Bean
	Consumer<BroadcastMessage<?>> messageChannelMaculaEventConsumer() {
		log.debug("[Macula] |- Bean [Consumer<BroadcastMessage<?>>] Auto Configure.");
		return (message) -> {
			if (log.isTraceEnabled()) {
				log.trace("[Macula] |- Message |- Receive [{}] -> from [{}] payload [{}]", message.getClass(), message.getSourceApplication(),
						message.getPayload());
			}
