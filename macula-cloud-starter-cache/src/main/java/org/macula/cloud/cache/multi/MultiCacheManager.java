...
import org.macula.cloud.cache.MultiCacheProperties;
...
public class MultiCacheManager implements CacheManager {
	...
	public MultiCacheManager(L1CacheManager l1CacheManager, L2CacheManager l2CacheManager, MultiCacheProperties properties) {
	...
	private Cache createMultiCacheByProperties(String name) {
		...
		if (config.isL1Enabled() && config.isL2Enabled() && l1CacheManager != null && l2CacheManager != null) {
			return new MultiAllCache(name, l1CacheManager.getL1Cache(name, config.getL1Spec()), l2CacheManager.getL2Cache(name, config.getL2Spec()));
		}
