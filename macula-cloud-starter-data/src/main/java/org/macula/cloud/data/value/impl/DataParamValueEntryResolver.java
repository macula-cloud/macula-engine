...
import org.macula.cloud.data.event.DataParamChangedEvent;
...
import org.macula.cloud.data.handle.impl.StaticOptionsDataHandler;
import org.macula.cloud.data.service.DataParamService;
import org.macula.cloud.data.util.DataSourceUtils;
import org.macula.cloud.data.value.ValueEntry;
import org.macula.cloud.data.value.ValueEntryResolver;
...
public class DataParamValueEntryResolver implements ValueEntryResolver, ApplicationListener<DataParamChangedEvent> {
	...
	@Override
	public ValueEntry resolve(String attribute, SubjectPrincipal userContext) {
	...
	public <T> Page<T> resolve(String code, SubjectPrincipal userContext, Pageable pageable) {
		DataParam dataParam = dataParamService.findByCode(code);
		String originTetxt = dataParam.getValue();
		DataSource dataSource = dataParam.getDataSource();
		Page<T> resolvedValue = null;
		if (dataSource == null) {
			DataHandlerChain dataHandleChain = new DataHandlerChain(CloudApplicationContext.getBean(StaticOptionsDataHandler.class));
			dataHandleChain.addInitialParameter(StaticOptionsDataHandler.TARGET_CLASS, dataParam.getDataType().getTypeClass());
			List<T> content = (List<T>) dataHandleChain.handle(originTetxt, userContext, null, null);
			resolvedValue = new PageImpl<T>(content, pageable == null ? PageRequest.of(0, Integer.MAX_VALUE) : pageable, content.size());
		} else {
			DataHandlerChain dataHandleChain = new DataHandlerChain(CloudApplicationContext.getBean(FreemarkerDataHandler.class),
					CloudApplicationContext.getBean(QueryParserDataHandler.class), CloudApplicationContext.getBean(QueryExecutorHandler.class));
			resolvedValue = (Page<T>) dataHandleChain.handle(originTetxt, userContext, DataSourceUtils.get(dataSource), pageable);
				...
				for (Object object : content) {
					if (object instanceof Map) {
						...
						if (convertEntry != null) {
							Object convertedValue = ConversionUtils.convertQuietly(convertEntry.getValue(), dataParam.getDataType().getTypeClass());
