package org.macula.cloud.data.handle.support;
...
import org.macula.cloud.data.util.DataSetUtils;
...
public class BeanPropertyRowMapper<T> extends org.springframework.jdbc.core.BeanPropertyRowMapper<T> {
	...
	private SubjectPrincipal userContext;
	...
	public void initContext(Map<String, Object> extractParams, SubjectPrincipal userContext) {
	...
	protected Object getColumnValue(ResultSet rs, int index, PropertyDescriptor pd) throws SQLException {
		...
		if (value != null && extractParams != null && extractParams.containsKey(labelKeyUpper)) {
			FieldOption option = DataSetUtils.createFieldOption((String) extractParams.get(labelKeyUpper), value.toString(), null, userContext);
			if (option != null) {
				beanWrapper.setPropertyValue(pd.getName() + DataParamTokenHandler.DATA_PARAM_KEY_SUFFIX, option.getLabel());
			}
		}
