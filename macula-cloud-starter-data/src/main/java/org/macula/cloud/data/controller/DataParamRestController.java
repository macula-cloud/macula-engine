package org.macula.cloud.data.controller;
...
public class DataParamRestController {
	...
	public List<Map<String, Object>> list(@PathVariable("code") String code, @RequestParam(required = false) Map<String, Object> params) {
		SubjectPrincipal userContext = SecurityUtils.getSubjectPrincipal();
		return DataSetUtils.createMappedOptions(code, userContext);
	}

	@ResponseBody
	@RequestMapping(value = "/{code}/pageable", method = RequestMethod.GET)
	public Page<?> page(@PathVariable("code") String code, @RequestParam(required = false) Map<String, Object> params, Pageable pageable) {
		SubjectPrincipal userContext = SecurityUtils.getSubjectPrincipal();
