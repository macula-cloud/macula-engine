...
public final class DomainQueryUtils {
	...
	public static <T> Page<T> query(EntityManager em, Class<?> domainClass, List<CommonCondition> conditions, Pageable pageable) {
		return query(em, StringUtils.EMPTY, domainClass, conditions, pageable);
	}

	public static <T> Page<T> query(EntityManager em, String select, Class<?> domainClass, List<CommonCondition> conditions, Pageable pageable) {
		...
		if (conditions != null) {
			for (CommonCondition commonCondition : conditions) {
				...
				if (criteriaType == null || !criteriaType.support(field.getType())) {
					throw new MaculaArgumentException("macula.base.data.conditions.criteriaType.notSupport", name, criteriaType);
				}
