...
public class DataSourceFactory {
	...
	public static final <T> T createTargetDataSource(org.macula.cloud.data.domain.DataSource ds) {
		if (ds != null) {
	...
	static {
		regist(DataSourceType.DATABASE, new DataSourceCreator<DataSource>() {
			@Override
			public DataSource createTargetDataSource(org.macula.cloud.data.domain.DataSource ds) {
				if (ds.isJndi()) {
					try {
		...
		regist(DataSourceType.LDAP, new DataSourceCreator<InitialDirContext>() {
			@Override
			public InitialDirContext createTargetDataSource(org.macula.cloud.data.domain.DataSource ds) {
				if (ds.isJndi()) {
					try {
						Context initCtx = new InitialContext();
