...
import org.macula.cloud.data.domain.DataArg;
import org.macula.cloud.data.domain.DataSet;
import org.macula.cloud.data.handle.DataHandlerChain;
import org.macula.cloud.data.handle.HandlerChainSerializer;
import org.macula.cloud.data.handle.impl.FreemarkerDataHandler;
import org.macula.cloud.data.handle.impl.QueryExecutorHandler;
import org.macula.cloud.data.handle.impl.QueryParserDataHandler;
import org.macula.cloud.data.handle.support.BeanPropertyRowMapper;
import org.macula.cloud.data.service.DataSetService;
import org.macula.cloud.data.value.impl.DataParamValueEntryResolver;
...
public final class DataSetUtils {
	 ...
	 * @return Page
	 */
	public static <T> Page<T> query(String sql, DataSource dataSource, SubjectPrincipal userContext, Object params, Pageable pageable) {
	...
	public static <T> Page<T> query(String sql, DataSource dataSource, SubjectPrincipal userContext, Object params, Pageable pageable,
		...
		DataHandlerChain chain = new DataHandlerChain(CloudApplicationContext.getBean(FreemarkerDataHandler.class),
				CloudApplicationContext.getBean(QueryParserDataHandler.class), CloudApplicationContext.getBean(QueryExecutorHandler.class));
		if (null != mappedParams) {
			for (Entry<String, Object> entry : mappedParams.entrySet()) {
				if (entry.getValue() instanceof Serializable) {
					chain.addInitialParameter(entry.getKey(), (Serializable) entry.getValue());
				} else {
					chain.addInitialParameter(entry.getKey(), String.valueOf(entry.getValue()));
				}
			}
		}
	 ...
	 * @return Page
	 */
	public static <T> Page<T> query(String code, SubjectPrincipal userContext, Object params, Pageable pageable) {
		return query(code, userContext, params, pageable, null);
	}

	public static <T> Page<T> query(String code, SubjectPrincipal userContext, Object params, Pageable pageable, Class<T> requiredType) {
	 ...
	 * @return Page
	 */
	public static <T> Page<T> query(DataSet dataSet, SubjectPrincipal userContext, Object params, Pageable pageable) {
		return query(dataSet, userContext, params, pageable, null);
	}

	public static <T> Page<T> query(DataSet dataSet, SubjectPrincipal userContext, Object params, Pageable pageable, Class<T> requiredType) {
				...
				if (paramValue instanceof List<?>) {
					...
					if (options == null || contains(convertedValue, options)) {
						if (convertedValue instanceof Serializable) {
							chain.addInitialParameter(argName, (Serializable) convertedValue);
						} else {
	...
	public static <T> Page<T> queryByDefaultContext(String sql, DataSource dataSource, Object params, Pageable pageable) {
		return query(sql, dataSource, SecurityUtils.getSubjectPrincipal(), params, pageable);
	}

	public static <T> Page<T> queryByDefaultContext(String sql, DataSource dataSource, Object params, Pageable pageable, Class<T> requiredType) {
		return query(sql, dataSource, SecurityUtils.getSubjectPrincipal(), params, pageable, requiredType);
	}
	...
	public static <T> Page<T> queryByDefaultContext(String code, Object params, Pageable pageable) {
		return query(code, SecurityUtils.getSubjectPrincipal(), params, pageable);
	}

	public static <T> Page<T> queryByDefaultContext(String code, Object params, Pageable pageable, Class<T> requiredType) {
		return query(code, SecurityUtils.getSubjectPrincipal(), params, pageable, requiredType);
	}
	...
	public static <T> Page<T> queryByDefaultContext(DataSet dataSet, Object params, Pageable pageable) {
		return query(dataSet, SecurityUtils.getSubjectPrincipal(), params, pageable);
	}

	public static <T> Page<T> queryByDefaultContext(DataSet dataSet, Object params, Pageable pageable, Class<T> requiredType) {
		return query(dataSet, SecurityUtils.getSubjectPrincipal(), params, pageable, requiredType);
	}
	...
	public static FieldOption createFieldOption(String dataParamCode, String dataCode, Map<String, Object> extraParams,
		...
		SubjectPrincipal extraSubjectPrincipal = userContext.addAdditionInfo(params);
		List<FieldOption> options = DataSetUtils.createFieldOptions(dataParamCode, extraSubjectPrincipal);
		if (options.size() == 1) {
	 ...
	 * @return List
	 */
	public static List<FieldOption> createFieldOptions(String dataParamCode, SubjectPrincipal userContext) {
		...
		for (Map<String, Object> map : paramValues) {
			...
			if (map.get(FieldOption.ID_PARAM) != null) {
				fieldOption = new FieldOption(map.get(FieldOption.CODE_PARAM).toString(), map.get(FieldOption.LABEL_PARAM).toString(),
						(map.get(FieldOption.ID_PARAM).toString()));
			} else {
				fieldOption = new FieldOption(map.get(FieldOption.CODE_PARAM), map.get(FieldOption.LABEL_PARAM).toString());
			}
			// Arron20151028 end
	 ...
	 * @return List
	 */
	public static List<Map<String, Object>> createMappedOptions(String dataParamCode, SubjectPrincipal userContext) {
	 ...
	 * @param pageable
	 */
	public static Page<Map<String, Object>> createMappedOptions(String dataParamCode, SubjectPrincipal userContext, Pageable pageable) {
		Page<Object> page = DataSetUtilsHolder.getDataParamValueEntryResolver().resolve(dataParamCode, userContext, pageable);
		...
		return new PageImpl<Map<String, Object>>(converted, PageRequest.of(page.getNumber(), page.getSize()), page.getTotalElements());
	}
	...
	@SuppressWarnings("unchecked")
	private static <T> Page<T> doChain(DataHandlerChain chain, String sql, SubjectPrincipal userContext, DataSource dataSource, Pageable pageable,
			Class<T> requiredType) {
